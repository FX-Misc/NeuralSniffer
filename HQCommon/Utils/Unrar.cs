 ...
 *  Modified by: Robin
 *  Company: Incode
 *  Version: 20090829   + 32 & 64 bit unrar.dll
 *                      + IDisposable pattern
 */
...
namespace Schematrix
{
	...
	#region Unrar PInvoke function declarations

	internal interface IUnrarPInvoke
	{
		IntPtr OpenArchive(ref Unrar.RAROpenArchiveData archiveData);
		IntPtr OpenArchiveEx(ref Unrar.RAROpenArchiveDataEx archiveData);
		int CloseArchive(IntPtr hArcData);
		int ReadHeader(IntPtr hArcData, ref Unrar.RARHeaderData headerData);
		int ReadHeaderEx(IntPtr hArcData, ref Unrar.RARHeaderDataEx headerData);
		int ProcessFile(IntPtr hArcData, int operation,
			[MarshalAs(UnmanagedType.LPStr)] string destPath, 
			[MarshalAs(UnmanagedType.LPStr)] string destName );
		void SetCallback(IntPtr hArcData, UNRARCallback callback, int userData);
		void SetPassword(IntPtr hArcData,
			[MarshalAs(UnmanagedType.LPStr)] string password);
	}
	// Unrar callback delegate signature
	internal delegate int UNRARCallback(uint msg, int UserData, IntPtr p1, int p2);

	internal class Unrar32PInvoke : IUnrarPInvoke
	{
		[DllImport("unrar32.dll")]
		private static extern IntPtr RAROpenArchive(ref Unrar.RAROpenArchiveData archiveData);
		IntPtr IUnrarPInvoke.OpenArchive(ref Unrar.RAROpenArchiveData archiveData)
		{
			return RAROpenArchive(ref archiveData);
		}

		[DllImport("unrar32.dll")]
		private static extern IntPtr RAROpenArchiveEx(ref Unrar.RAROpenArchiveDataEx archiveData);
		IntPtr IUnrarPInvoke.OpenArchiveEx(ref Unrar.RAROpenArchiveDataEx archiveData)
		{
		...
		int  IUnrarPInvoke.CloseArchive(IntPtr hArcData)
		{
		...
		int  IUnrarPInvoke.ReadHeader(IntPtr hArcData, ref Unrar.RARHeaderData headerData)
		{
			return RARReadHeader(hArcData, ref headerData);
		}

		[DllImport("unrar32.dll")]
		private static extern int RARReadHeaderEx(IntPtr hArcData, ref Unrar.RARHeaderDataEx headerData);
		int  IUnrarPInvoke.ReadHeaderEx(IntPtr hArcData, ref Unrar.RARHeaderDataEx headerData)
		{
			return RARReadHeaderEx(hArcData, ref headerData);
		}
		...
		private static extern int RARProcessFile(IntPtr hArcData, int operation,
			[MarshalAs(UnmanagedType.LPStr)] string destPath, 
			[MarshalAs(UnmanagedType.LPStr)] string destName );
		int  IUnrarPInvoke.ProcessFile(IntPtr hArcData, int operation, string destPath, string destName)
		{
		...
		void  IUnrarPInvoke.SetCallback(IntPtr hArcData, UNRARCallback callback, int userData)
		{
		...
		private static extern void RARSetPassword(IntPtr hArcData,
			[MarshalAs(UnmanagedType.LPStr)] string password);
		void  IUnrarPInvoke.SetPassword(IntPtr hArcData, string password)
		{
			RARSetPassword(hArcData, password);
		}
	}

	internal class Unrar64PInvoke : IUnrarPInvoke
	{
	...
	{
		static readonly IUnrarPInvoke PInvoke
			= (IntPtr.Size == 4) ? (IUnrarPInvoke)new Unrar32PInvoke() 
								 : new Unrar64PInvoke();
		...
		protected virtual void Dispose(bool p_notFromDtor)
        {
			...
			{
				PInvoke.CloseArchive(this.archiveHandle);
		...
		public string Password
		{
			...
			set
			{
				this.password=value;
				if(this.archiveHandle!=IntPtr.Zero)
					PInvoke.SetPassword(this.archiveHandle, value);
			}
		}
		...
		public void Close()
		{
			...
			// Close archive
			int result=PInvoke.CloseArchive(this.archiveHandle);
		...
		public void Open(string archivePathName, OpenMode openMode)
		{
			...
			// Open archive
			handle=PInvoke.OpenArchiveEx(ref openStruct);
			...
			// Set callback
			PInvoke.SetCallback(this.archiveHandle, this.callback, this.GetHashCode());
			...
			if(this.password.Length!=0)
				PInvoke.SetPassword(this.archiveHandle, this.password);
		...
		public bool ReadHeader()
		{
			...
			// Read next entry
			currentFile=null;
			int result=PInvoke.ReadHeaderEx(this.archiveHandle, ref this.header);
		...
		public void Skip()
		{
			int result=PInvoke.ProcessFile(this.archiveHandle, (int)Operation.Skip, string.Empty, string.Empty);
		...
		public void Test()
		{
			int result=PInvoke.ProcessFile(this.archiveHandle, (int)Operation.Test, string.Empty, string.Empty);
		...
		{
			int result=PInvoke.ProcessFile(this.archiveHandle, (int)Operation.Extract, destinationPath, destinationName);
